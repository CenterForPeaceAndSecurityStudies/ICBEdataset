...
authors:
  - name: Rex W. Douglass
    thanks: Correspondence should be addressed to Rex W. Douglass at rexdouglass@gmail.com.
...
corresponding_author:
  text: "Correspondence should be addressed. E-mail: rexdouglass@gmail.com"
...
nocite: |
  @fazalStateDeathPolitics2011, @spruytSovereignStateIts1996, @haffarEmergentPeacemakersCataloguing2002, @bushDensityDeclineFounding2019, @benyehudaEthnicActorsInternational2006, @lacinaExplainingSeverityCivil2006, @mcnabbcochranMeasuringMilitaryEffectiveness2017, @carafanoMeasuringMilitaryPower2014, @goertzMeasuringMilitaryAllocations1986, @gartzkeCrossDomainDeterrenceStrategy2019,  @lindsayPoliticsManyOther2020, @carterStrategyTerritorialConflict2010, @brecher_study_1997, @goldgeierPsychologyInternationalRelations2001, @steinEvaluatingWarOutcomes1980, @sullivanWarAimsWar2007, @ramsayInformationUncertaintyWar2017, @yarhi-miloEyeBeholderHow2013, @oneillInternationalNegotiationConceptual2018, @zartmanEscalationNegotiationInternational2005, @sechserMilitarizedCompellentThreats2011, @powellBargainingTheoryInternational2002, @goemansIntroducingArchigosDataset2009,  @powellGlobalInstancesCoups2011, @chenowethIntroducingNonviolentAction2019, @eckOneSidedViolenceCivilians2007, @lafreeIntroducingGlobalTerrorism2007, @beardsleyMediationDilemma2011, @giblerMeasuringAlliancesCorrelates2004, @owsiakInternationalBorderAgreements2018, @leedsAllianceReliabilityTimes2003, @leedsDomesticPoliticalInstitutions1999, @yarhi-miloArmAllyPatron2016, @laiEffectsDifferentTypes2004, @allenUSGlobalMilitary2021, @fortnaPeaceTime2018, @minInterstateWarBattle2021, @sarkeesResortWar181620072010, @reiterRevisedLookInterstate2016, @frederickIssueCorrelatesWar2017
...
#  - \renewenvironment{references}{}{}
  - \renewcommand{\bibliography}[1]{} # removes the superflusous references section
#  - \renewcommand{\bibliographystyle}[1]{} # might be an alternative to renewcommand bibliography
#  - \renewenvironment{bibliography}{}{} # this drops the references but leaves an unnecessary paper. bib

#https://tex.stackexchange.com/questions/131646/algorithm2e-command-algorithm-already-defined
---

```{r options, echo=F, cache=F, messages=F, results='hide', warnings=F, include=FALSE}
...
fromscratch = FALSE #Set to TRUE to repull google docs
```

```{r package-load, eval=T, echo=F, cache=F}
#library(pacman)
...
[^1]: See work on crises [@brecherCrisesWorldPolitics1982; @beardsleyInternationalCrisisBehavior2020], militarized disputes [@palmerMID5Dataset20112021; @giblerInternationalConflicts181620102018], wars [@sarkeesResortWar181620072010; @reiterRevisedLookInterstate2016], organized violence [@ralphsundbergUCDPGEDCodebook2016; @daviesOrganizedViolence19892022], political violence [@raleighIntroducingACLEDArmed2010], sanctions [@felbermayrGlobalSanctionsData2020], and international agreements [@kinneDefenseCooperationAgreement2020; @owsiakInternationalBorderAgreements2018], dispute resolution [@frederickIssueCorrelatesWar2017], and diplomacy [@moyerWhatAreDrivers2020; @sechserMilitarizedCompellentThreats2011].

[^2]: See @beielerGeneratingPoliticalEvent2016; @boscheeICEWSCodedEvent2015; @brandtPhoenixRealTimeEvent2018; @grantOUEventData2017; @liComprehensiveSurveySchemabased2021. On event extraction from images and social media see @zhangCASMDeepLearningApproach2019 and @steinert-threlkeldFutureEventData2019.

<!-- removedcites @barariDemocracyTradePolicy; @vabulasCooperationAutonomyBuilding2021; @hegreIntroducingUCDPCandidate2020; @haltermanExtractingPoliticalEvents2020; @maozDyadicMilitarizedInterstate2019; @vabulasCooperationAutonomyBuilding2021; -->
...
<!-- removedcites @jamesWhatWeKnow2019; @iakhnisCrisesWorldPolitics2019; @brecherEscalationWarTwentieth2000 -->
...
Our knowledge of any historical episode, including the participants and their preferences, behaviors, and beliefs are only indirectly observed from historical records that most often take the form of unstructured natural language text. Despite its complexity, all international interactions fundamentally involve a finite set of actors expressing their interests through at least theoretically observable behaviors. So how can scholars abstract and measure discrete events about a historical episode in international relations? Scholars have recently sought to produce a structured account of world events by combining the unstructured corpus of historical records with informative priors about international relations. The result can produce novel insight from a systematic account of political events. The easiest way to convey the desired product is with an example. Figure 1 shows a narrative account of the Cuban Missile Crisis (1962) in natural language sentences alongside a mapping to discrete machine-readable abstractive events. From this, scholars can identify similarities and differences across events like what foreign policy actions deter versus inflame [@jervisCooperationSecurityDilemma1978; @glaserCausesConsequencesArms2000], when third parties mediate [@haffarEmergentPeacemakersCataloguing2002; @quinnPowerPlayMediation2006], and how actors communicate resolve [@tragerDiplomacyWarPeace2016; @luptonReexaminingReputationResolve2018]. Identifying patterns of international interactions is not just an inherently interesting enterprise; it is a necessary precondition to important efforts to predict where policymakers should turn their attention to improve global welfare [@wardLearningSteppingFuture2013; @begerReassessingRoleTheory2021].

```{r cuban-case-study-precision, eval=T, results='hide', echo=F, include=T, message=F, cache=F, warning=F}
# AG temp edit: revert by setting eval=T DONE
...
for(i in 1:nrow(temp)){
  temp$is_subsstring[i] <-   stringr::str_detect(temp$value_markdown %>% tolower() %>% stringr::str_replace_all('[^a-z0-9]',"") ,
                                                                                pattern = stringr::fixed(temp$value_markdown[i] %>% tolower() %>% stringr::str_replace_all('[^a-z0-9]',"") )
                                                                              ) %>% replace_na(0) %>%  sum()
}
temp <-temp %>%
        mutate(value_markdown = ifelse(is_subsstring<=1,value_markdown,'')) %>%
        group_by(sentence_number_int_aligned) %>%
          filter(value_markdown!='' | length(value_markdown)==1) %>% # also added below to handle sentence 6
        ungroup() %>%
...
temp <- temp %>% dplyr::mutate(value_markdown = value_markdown %>% 
            stringr::str_replace_all(stringr::fixed("/home/tlscherer/Desktop/ICBEdataset/replication_data/in/"), stringr::fixed(""))) # %>% 
            # dplyr::mutate(value_markdown = value_markdown %>% replace_emoticons)
...
#ft_cuba_precision %>% flextable::save_as_image(path=here::here("replication_paper","psrm_draft","case_study_cuban_precision.png"), zoom=1) # webshot = "webshot2"
temp <- NULL
```

![Comparison of a natural language and machine-readable abstractive account of the Cuban Missile Crisis (1962). The text on the left is a summary of the event from the ICB Crisis Narrative. The mapping on the right shows the corresponding ICBe coding](case_study_cuban_precision.png){#fig-cubanarrative width="7in" height="8in"}
...
The rows in Table 1 represent the types of information we expect to find in international relations and forms the basis for our proposed ontology. We created this ontology by performing of the corpus and identifying named entities and verbs mentioned in the text. To identify possible behaviors, we matched verbs to the most likely definition found in Wordnet [@millerWordNetLexicalDatabase1995], tallied (SI Appendix 1.2), and then aggregated them into a smaller number of behaviors balancing conceptual detail with manageable sparsity for human coding, informed by existing conceptual ("Literature" column) and measurement research (discussed below). We used the ICB project actor level data to identify likely actors for each crisis and location options relative to each actor. For behavior, actor, and location, coders could write-in a value if the given options were insufficient. The codebook lists 11 behaviors added post-coding as coders flagged events that were not captured by the initial ontology (e.g. propaganda).

As we are not the first to attempt to measure international relations in a structured manner, the columns of Table 1 compare the ontological coverage of ICBe to existing state of the art systems in production and with global coverage. We choose these datasets and models as they represent frequently used and reputable efforts to structure and describe historical events of interest to scholars of international politics. The first column starts with our contribution, ICBe, alongside other event-level datasets including CAMEO dictionary lookup-based systems (Historical Phoenix [@althausClineCenterHistorical2019]; ICEWS [@boscheeICEWSCodedEvent2015]; Terrier [@grantOUEventData2017]), the Militarized Interstate Disputes Incidents dataset, the UCDP-GED dataset [@daviesOrganizedViolence19892022; @sundbergIntroducingUCDPGeoreferenced2013], and ACLED [@raleighIntroducingACLEDArmed2010].[^3] The final set of columns compares episode-level datasets beginning with the original ICB project [@brecherInternationalCrisisBehavior2021; @brecherCrisesWorldPolitics1982; @beardsleyInternationalCrisisBehavior2020]; the Militarized Interstate Disputes dataset [@palmerMID5Dataset20112021; @giblerInternationalConflicts181620102018], and the Correlates of War [@sarkeesResortWar181620072010]. We include episode-level datasets as they remain and common and trusted tool for analyzing international relations, and because ICBe is unique among event-level datasets as events are matched to crises and so can be aggregated to the episode level. There is imperfect overlap concerning their intended depth and scope of coverage; 'international crises' are similar, but not identical to, 'interstate wars' and 'militarized interstate disputes' which differ yet again from 'individual events of organized violence' and 'non-violent action'. Even like-concepts require care in comparison, as an 'aim' in ICBe is the same as in MIPS, but an 'alert' in ICBe is not the same as an 'alert' in MID.

[^3]: Other related datasets that insufficiently overlap ICBe's domain for comparison include BCOW [@lengMilitarizedInterstateCrises1988], WEIS [@mcclellandWorldEventInteraction1978], CREON [@hermannComparativeResearchEvents1984], CASCON [@bloomfieldCASCONIIIComputeraided1989], SHERFACS [@shermanSHERFACSCrossParadigmHierarchical2000], Real-Time Phoenix [@brandtPhoenixRealTimeEvent2018], and COfEE [@balaliCOfEEComprehensiveOntology2021] (see histories in @merrittMeasuringEventsInternational1994 and @schrodtTwentyYearsKansas2006).
...
<!-- removedcites @braithwaiteMIDLOCIntroducingMilitarized2010; @braithwaiteCodebookMilitarizedInterstate2009; @hegreIntroducingUCDPCandidate2020, @ralphsundbergUCDPGEDCodebook2016-->
...
```{r, eval=T, echo=F, results='asis', include=T, message=F, cache=F, warning=F, ft.arraystretch=1}
...
if(fromscratch){googlesheets4::gs4_deauth()}

if(fromscratch){
  lit_review_pull <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/14RHRKgQGHj7Zr4_xt4fq2bIE7mDNayLKj89tYmOOfHQ/edit?usp=sharing", sheet="DatasetsComparisonLitReview") #%>% janitor::clean_names() 
  saveRDS(lit_review_pull ,
            file=paste0(here::here(),"/replication_data/out/lit_review_pull.Rds")) 
} else {lit_review_pull <- readRDS( paste0(here::here(), "/replication_data/out/lit_review_pull.Rds"))}
...
lit_review_alt <- lit_review_pull %>% 
                  mutate(Literature = paste0("",Literature %>% stringr::str_replace("; {0,}$","") ,"")) %>%
                  mutate(Literature = paste0("",Literature %>% stringr::str_replace_all(";",",") ,"")) %>%
                  #mutate(Literature = paste0("  [",Literature %>% stringr::str_replace("; {0,}$","") ,"]")) %>%
                  #mutate(Concept= paste0("**", Concept, "**", Literature)) %>%
                  mutate(Concept= paste0(Concept)) %>%
                  mutate(Literature = Literature %>% stringr::str_replace_all(stringr::fixed("NA"),"") ) %>%
                  mutate(Concept = Concept %>% stringr::str_replace(stringr::fixed(" ["),stringr::fixed(" - ["))) %>%
                  dplyr::select(-Codebook, -`Cameo Codes`, -`ICB Codes`, Literature, -`ICB Corpus`, -`Removed Literature`) # %>% 
                  # dplyr::rename(`Illustrative Literature` = Literature)
...
r_unarmed= ( lit_review_pull$...1 %in% "Do - Unarmed" %>% sum() ) + r_say
r_armed= ( lit_review_pull$...1 %in% "Do - Armed" %>% sum() ) + r_unarmed
ft <- NULL
...
litreview_ft_psrm <- lit_review_alt %>%
      flextable::as_flextable() %>%
      ...
      flextable::fontsize(size = 6.5, part = "all") %>%
      #Banding
      bg( i = bands, j=2:k, part = "body", bg = "#EFEFEF") %>% 
  
      flextable::rotate(i=1, j=2:k, rotation="tbrl", part="header") %>%
      flextable::height(i=1, height=100, part="header") %>% 
      ...
      merge_at(i = 1:r_intro, j = 1, part = "body") %>%  
      ...
      merge_at(i = (r_intro+1):r_players, j = 1, part = "body") %>%  
      #pieces
      hline(i=c(r_pieces), border = NULL, part = "body") %>% 
      merge_at(i = (r_players+1):r_pieces, j = 1, part = "body") %>%  
      #think
      hline(i=c(r_think), border = NULL, part = "body") %>% 
      merge_at(i = (r_pieces+1):r_think, j = 1, part = "body") %>%  
      #say
      hline(i=c(r_say), border = NULL, part = "body") %>% 
      merge_at(i = (r_think+1):r_say, j = 1, part = "body") %>%
      #Do
      #merge_at(i = (r_say+1):r_armed, j = 1, part = "body") %>%  

      #n_unarmed
      hline(i=c(r_unarmed), j = 1:k, part = "body") %>% 
      merge_at(i = (r_say+1):r_unarmed, j = 1, part = "body") %>%  
      #n_armed
      #hline(i=c(r_armed), border = NULL, part = "body") %>% 
      merge_at(i = (r_unarmed+1):r_armed, j = 1, part = "body") %>%  

      #set_table_properties(layout="autofit")  %>%
      vline( i = 1, j = c(3,10), border = NULL, part = "header") %>%
      vline( i = 1:(n), j = 1, border = NULL, part = "body")  %>%
      vline( i = 1:(n), j = c(3,10), border = NULL, part = "body")  %>%
  
      # flextable::bold(i = 1:n, j=2, bold = TRUE, part = "body") %>%
      
      flextable::width(j = 1, width=0.1)  %>%
      flextable::width( j = 2, width=1.8) %>% # change width for inline citations for politicalanalysis (4 works, might have extra room)
      flextable::width( j = 3, width=1.5) %>% # change width for inline citations for politicalanalysis (4 works, might have extra room)
      flextable::width( j = (4):k, width=0.25) %>%
      flextable::line_spacing( space = 1.0, part = "body") %>% #i=1:nrow(lit_review_alt),
      flextable::padding(padding = 0, part = "body") %>%
      flextable::rotate(j=1, rotation="tbrl",part="all", align="center") %>%
...
#litreview_ft_psrm  %>% saveRDS(paste0(here::here(), '/replication_paper/tables/litreview_ft_psrm.Rds'))
litreview_ft_psrm %>% 
...
# litreview_ft_psrm %>% 
     #flextable::flextable_to_rmd()
#     save_as_image(path=here::here("replication_paper", "psrm_draft", "litreview_ft.png"), zoom=1, webshot = "webshot2") # webshot = "webshot2"
```
...
\doublespacing
```
```{r coder-counts, eval=T, results='hide', echo=F, include=T, message=F, cache=F}
...
[^6]: See the full codebook on Github Repository [ICBEventData](https://urldefense.com/v3/__https://github.com/CenterForPeaceAndSecurityStudies/ICBEventData__;!!Mih3wA!WxDJtEczKfxGTh0S2Krunap8ReymFEL5iTWaSfOHeqlSdyfRx77zmjBSWO1OAm13$).

A unique feature of the ontology is that thought, speech, and do behaviors can be nested into combinations, e.g. an offer for the U.S.S.R. to remove missiles from Cuba in exchange for the U.S. removing missiles from Turkey. Through compounding, the ontology can capture what players were said to have known, learned, or said about other specific fully described actions. Two other novel features of our data -- its re-programmamable coding and its inclusion of thought, speech, and action -- intersect to provide a unique contribution.

First, re-programmable coding means scholars can make modifications to the ontology we provide and re-run the code script to produce a modified dataset. For example, if thoughts and speech acts are immaterial for a particular research question, those higher-level categories can be modified in a way that does not produce inevitable downstream effects on the rest of the coding. This is necessary because unlike existing datasets where variable values are largely independent, the nested nature of the ICBe variables means filtering out speech acts also requires filtering out variables about the actors, locations, and timing of those speech acts.

Second, no existing event data distinguishes thoughts, speeches, and actions. In fact, most only try to code actions and entirely omit thoughts and speech acts despite recognition of their importance in international politics [@smithInternationalCrisesDomestic1998]. Scholars have opted against coding thoughts and speech acts because of a lack of confidence the full universe could be readily observed and consequently at least theoretically be included.[^7] But the perfect should not be the enemy of the good, and measurement challenges are only overcome after an initial attempt to estimate difficult-to-observe concepts of interest. The ICB narratives are one of the better sources for this endeavor due to the consistent use of high-quality primary source material that takes advantage of qualitative methods well-suited to identifying thoughts and speech acts like archival work and expert interviews. Important questions about potential bias remain like whether a thought was known at the time by those involved or only recorded long afterwards by historians, but data validation required there to be data to be validated. By design, existing datasets largely omit thoughts and speech acts and also treat the resulting dataset as the static product of hardwired coding decisions. In allowing scholars to identify actions, thoughts, and speech acts as distinct characteristics of international crises while also granting flexibility about the inclusion of a non-random sample of novel variables, our approach is a notable improvement over the state of the art.

[^7]: Even the coding of overt actions like MIDs is not without contention [@giblerInternationalConflicts181620102018].

Each crisis was typically assigned to 2 expert coders and 2 novice coders with an additional tie-breaking expert coder assigned to sentences with high disagreement.[^8] For the purposes of measuring intercoder agreement and consensus, we temporarily disaggregate the unit of analysis to the Coder-Crisis-Sentence-Tag (n=`r icb_long_clean %>% nrow() %>% scales::comma()`), where a tag is any unique piece of information a coder can associate with a sentence such as an actor, date, behavior, etc. We then aggregate those tags into final events (n=`r ICBe_events_agreed %>% nrow() %>% scales::comma()`), using a consensus procedure (SI Appendix 2.2) that requires a tag to have been chosen by at least one expert coder and either a majority of expert or novice coders. This screens noisy tags that no expert considered possible but leverages novice knowledge to tie-break between equally plausible tags chosen by experts. Requiring sentence-tag matching may underestimate agreement but minimizes the inclusion of noise and allows for additional validation. Once filtered for agreement, we find `r codings_long_agreement %>% filter(varname_normalized %>% str_detect("actor")) %>% pull(value_normalized) %>% unique() %>% length()` actors and `r codings_long_agreement %>% filter(varname_normalized %>% str_detect("act_cooperative|interact_increasecoop|act_uncooperative|interact_decreasecoop|act_escalate|interact_escalate|act_deescalate|interact_deescalate|sayintkind|thinkkind")) %>% filter(grepl("[A-Za-z]", value_normalized)) %>% pull(value_normalized) %>% unique() %>% length()` different behaviors: `r codings_long_agreement %>% filter(varname_normalized %>% str_detect("thinkkind")) %>% filter(grepl("[A-Za-z]", value_normalized)) %>% pull(value_normalized) %>% unique()  %>% length()` thought, `r codings_long_agreement %>% filter(varname_normalized %>% str_detect("sayintkind")) %>% filter(grepl("[A-Za-z]", value_normalized)) %>% pull(value_normalized) %>% unique()  %>% length()` speech, and `r codings_long_agreement %>% filter(varname_normalized %>% str_detect("act_cooperative|interact_increasecoop|act_uncooperative|interact_decreasecoop|act_escalate|interact_escalate|act_deescalate|interact_deescalate")) %>% pull(value_normalized) %>% unique() %>% length()` actions.
...
```{r recall-cuba, eval=T, results='hide', echo=F, include=T, message=F, cache=F, warning=F, ft.arraystretch=0.75}
#Lit review
...
if(fromscratch){
  case_study_cuban <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1NuRWFB1HEbQbJu_JoEzJ9WYk7kfwwCryF7wBfKPT6uc/edit?usp=sharing", sheet="final") %>% janitor::clean_names() 
  saveRDS(case_study_cuban, file=paste0(here::here(),"/replication_data/out/case_study_cuban.Rds")) 
} else {case_study_cuban <- readRDS( paste0(here::here(), "/replication_data/out/case_study_cuban.Rds"))}
...
ft   %>% 
    #flextable::flextable_to_rmd()
    save_as_image(path=here::here("replication_paper", "psrm_draft", "case_study_cuban_recall.png"), zoom=1, webshot = "webshot2") # webshot = "webshot2"

```

```{r recall-crimea, eval=T, results='hide', echo=F, include=T, message=F, cache=F, warning=F, ft.arraystretch=0.75}
library(dplyr)
library(googlesheets4) #Do this high up so you get the prompt early
library(lubridate, warn.conflicts = FALSE)

if(fromscratch){
  case_study_crimea <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1YesAx1CkYCgrEi_WVJ9aho60HesWA6p-3XMma9IzBrI/edit?usp=sharing", sheet="final") %>% janitor::clean_names() 
  saveRDS(case_study_crimea, file=paste0(here::here(),"/replication_data/out/case_study_crimea.Rds")) 
} else {case_study_crimea <- readRDS( paste0(here::here(), "/replication_data/out/case_study_crimea.Rds"))}
...
replace_emoticons_crimea <- function(x){
      x %>%
            ...
            #
            str_replace_all('🇪🇺', paste0(" ![](", here::here(), "/replication_data/in/flags_small//Q458.png) "))
                            }
...
ft   %>% 
    #flextable::flextable_to_rmd()
    save_as_image(path=here::here("replication_paper", "psrm_draft", "case_study_crimea_recall.png"),
                  zoom=1, webshot = "webshot2") # webshot = "webshot2"
```

```{r recall-combined, eval=F, results='hide', echo=F, include=T, message=F, cache=F, warning=F, ft.arraystretch=0.75}

# AG temp edit: revert by setting 'eval=T'
...
ft <- temp3  %>%
      ...
      add_header_row(values = c("Cuban Missile Crisis (1962)", "", "Crimea-Donbas (2014)"), colwidths = c(kleft, 1,kright) ) %>%
...
ft   %>% 
    #flextable::flextable_to_rmd()
    save_as_image(path=here::here("replication_paper", "psrm_draft", "recall_cuban_and_crimea.png"), zoom=1, webshot = "webshot2") # webshot = "webshot2"
```

```{r recall-all, eval=T, results='hide', echo=F, include=T, message=F, cache=F, warning=F, ft.arraystretch=0.75}
...
ggsave(file=paste0(here::here(), '/replication_paper/psrm_draft/p_recall.png'),
       plot = p_recall ,
       width=5, height=3.0)

library(magick)

tiger <- image_read(here::here("replication_paper", "psrm_draft", "recall_cuban_and_crimea.png"))
tiger2 <- image_read(paste0(here::here(), '/replication_paper/psrm_draft/p_recall.png'))

combined <- image_composite(tiger, image_scale(tiger2, "x290"), offset = "+475+650")
image_write(combined, here::here("replication_paper", "psrm_draft", "recall_cuban_and_crimea_andcounts.png"))
```


![Recall comparison of two cases across existing state of the art efforts. Higher y-axis values represent higher recall and higher x-axis values represent number of times that detail is mentioned across the full corpus used to construct the synthetic narrative.](p_recall.png){#fig-recall-cases}
...
if(fromscratch){
  actor_translator <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1a7Id0Zg41PTKEv74H_KjiJzhMak2jGh0leT7B8oVWdI/edit?usp=sharing", sheet="actor_translator") #%>% janitor::clean_names() 
  saveRDS(actor_translator,
            file=paste0(here::here(),"/replication_data/out/actor_translator.Rds")) 
} else {actor_translator <- readRDS( paste0(here::here(), "/replication_data/out/actor_translator.Rds"))}
...
```{r precision-combined, eval=T, results='hide', echo=F, include=T, message=F, cache=F, warning=F}
library(ggplot2)
library(cowplot)

p_196_icbe <- icbe_crisismaps(thiscrisis=196) +
    labs(
      #title = plot_title ,
      #subtitle = subtitle ,
      caption = NULL
...
ratio=.75
height=18
save_plot(file=paste0(here::here(), '/replication_paper/psrm_draft/p_196_icbe.png'),
          p_196_icbe, base_height = 18, base_width = height*ratio)
...
ratio=.75
height=18
save_plot(file=paste0(here::here(), '/replication_paper/psrm_draft/p_471_icbe.png'),
          p_471_icbe, base_height = 18, base_width = height*ratio)
...
ratio=1.5
height=18
save_plot(file=paste0(here::here(), '/replication_paper/psrm_draft/p_precision_combined.png'),
          p_precision_combined, base_height = 18, base_width = height*ratio)

```

```{r sentences-unique, eval=T, results='hide', echo=F, include=T, message=F, cache=F, warning=F}
library(flextable)
library(ftExtra)
library(tidyverse)
...
```{r reticulate, eval=T, results='hide', echo=F, include=T, message=F, cache=F, warning=F}
...
```{python sentence-transformers, eval=fromscratch, results='hide', echo=F, include=T, message=F, cache=F, warning=F}
...
```{r sentence-embeddings, results='hide', echo=F, include=T, message=F, cache=F, warning=F}
if(fromscratch){
...
```{r umap, results='hide', echo=F, include=T, message=F, cache=F, warning=F, eval=fromscratch}
...
# AG temp edit: revert by deleting 'eval=F' above DONE
...
```{r sentence-embeddings-dendro, eval=fromscratch, results='hide', echo=F, include=T, message=F, cache=F, warning=F, ft.arraystretch=0.75}
...
ggsave(file=paste0(here::here(), '/replication_paper/psrm_draft/p_semantic_embeddings.png'), plot = p_semantic_embeddings, width=12, height=8)
...
ggsave(file=paste0(here::here(), '/replication_paper/psrm_draft/p_dendro.png'), plot = p_dendro, width=12, height=10)
...
![Crisis map for the Cuban Missile Crisis. The start of the crisis is at the top and end of the crisis is the bottom, with each actor in a column with labeled points identifying their speeches, actions, and thoughts.](p_196_icbe.png){#fig-cuba-crisismap width="5.5in"}
...
![Synthetic narratives combine several thousand accounts of each crisis into a single timeline of events, taking only those mentioned in at least 5 or more documents. Checkmarks represent whether that event could be hand matched to any detail in the ICB corpus, ICBe dataset, or any of the other event datasets (SI Appendix 3.2 and 3.3).](case_study_crimea_recall.png){#fig-recall-crimea}
...
```{r precision, eval=fromscratch, results='hide', echo=F, include=T, message=F, cache=F, warning=F, ft.arraystretch=0.75}
...
icews_clean_subset_top10_day <- icews_clean_subset_top10_day %>% 
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all("/mnt/8tb_a/rwd_github_private/ICBEdataset", here::here())) %>%
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(" ", "")) %>%
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all("VladimirPutin", "Putin")) %>%
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("png))"), stringr::fixed("png)"))) %>%
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("(!"), stringr::fixed("!"))) %>%
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("ViktorYanukovych->![](/"), stringr::fixed("Yanukovych->.![](/"))) %>% 
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("Putin->![](/"), stringr::fixed("Putin->![](/"))) %>%  
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("Military![]"), stringr::fixed("Military-![]"))) %>%
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("Police![]"), stringr::fixed("Police.![]"))) %>%
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("Citizen![]"), stringr::fixed("Citizen..![]"))) %>%
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("ArmedRebel![]"), stringr::fixed("ArmedRebel-![]"))) %>%
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("Bank![]"), stringr::fixed("Bank.![]"))) %>%
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("Business![]"), stringr::fixed("Business.![]"))) %>%
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("Insurgent![]"), stringr::fixed("Insurgent-![]"))) %>%
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("Separatist![]"), stringr::fixed("Separatist-![]"))) %>%  
  dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("BarackObama->![](/"), stringr::fixed("Obama->.![](/"))) 

#Separatist(![](/home/tlscherer/Desktop/ICBEdataset/replication_data/in/flags_small//Q212.png))->Military(![](/home/tlscherer/Desktop/ICBEdataset/replication_data/in/flags_small//Q212.png))
  
  #dplyr::mutate(dyad_markdown = dyad_markdown %>% stringr::str_replace_all(stringr::fixed("(!"), stringr::fixed("--!"))) %>%

#/replication_data/in/flags_small//Q159.png
...
save_plot(file=paste0(here::here(),
                      '/replication_paper/psrm_draft/p_precision_icews.png'),
          p_ICEWS, base_height = 10, base_width = 14)

#}
```

![The unit of analysis is the dyad-day. Top 10 most active dyads per category shown. Red text shows events from the synthetic narrative relative to that event category. Blue bars indicate an event recorded by ICEWs for that dyad on that day.](p_precision_icews.png){#fig-precision-icews width="7in" height="5in"}
...
This article's data, supplementary appendix, replication material, and visualizations of every historical episode are available on the GitHub repository [ICBEventData](https://urldefense.com/v3/__https://github.com/CenterForPeaceAndSecurityStudies/ICBEventData__;!!Mih3wA!WxDJtEczKfxGTh0S2Krunap8ReymFEL5iTWaSfOHeqlSdyfRx77zmjBSWO1OAm13$) and through the companion website crisisevents.org.
