---
title: "Introducing ICBe: Supplementary Information"
...
\clearpage
# Corpus and Cases
## ICBe Crises and Summary Information {#app:crises}
...
temp_all <-  ICBe_crises_markdown %>% ungroup() %>% dplyr::select(
  ...
  dplyr::mutate(actors = actors %>% stringr::str_replace_all(stringr::fixed("/home/tlscherer/Desktop/ICBEdataset/replication_data/in/"), stringr::fixed(""))) %>%
  dplyr::mutate(actors = actors %>% stringr::str_replace_all(stringr::fixed("Columbia;Cuba;Mexico;United States;Uruguay"), stringr::fixed("![](flags_small//Q739.png);![](flags_small//Q241.png);![](flags_small//Q96.png)"))) %>%
  dplyr::mutate(actors = actors %>% stringr::str_replace_all(stringr::fixed("Belgium;France;Germany;Italy;United Kingdom"), stringr::fixed(""))) %>%
  dplyr::mutate(actors = actors %>% stringr::str_replace_all(stringr::fixed("France;United Kingdom"), stringr::fixed(""))) %>%
  dplyr::mutate(actors = actors %>% stringr::str_replace_all(stringr::fixed(";Soviet Union;United States of America"), stringr::fixed(""))) %>%
  dplyr::mutate(actors = actors %>% stringr::str_replace_all(stringr::fixed(";Saudi Arabia"), stringr::fixed(""))) %>%
  dplyr::mutate(units_domains = units_domains %>% stringr::str_replace_all(stringr::fixed("/home/tlscherer/Desktop/ICBEdataset/replication_data/in/"), stringr::fixed(""))) %>%
  dplyr::mutate(forces_fatalities = forces_fatalities %>% stringr::str_replace_all(stringr::fixed("/home/tlscherer/Desktop/ICBEdataset/replication_data/in/"), stringr::fixed("")))
## DOUBLE CHECK ABOVE
...
crisis_text_counts_ft1 <- temp_all %>% ungroup() %>% filter(crisno %in% c(1:60)) %>% convert_to_ft() 
#crisis_text_counts_ft1 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "crisis_text_counts_ft1.png"), zoom=1)
crisis_text_counts_ft1

crisis_text_counts_ft2 <- temp_all %>% ungroup() %>% filter(crisno %in% c(61:120) ) %>% convert_to_ft()  
#crisis_text_counts_ft2 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "crisis_text_counts_ft2.png"), zoom=1)
crisis_text_counts_ft2

crisis_text_counts_ft3 <- temp_all %>% ungroup() %>% filter(crisno %in% c(121:180) ) %>% convert_to_ft() 
#crisis_text_counts_ft3 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "crisis_text_counts_ft3.png"), zoom=1)
crisis_text_counts_ft3

crisis_text_counts_ft4 <- temp_all %>% ungroup() %>% filter(crisno %in% c(181:240) ) %>% convert_to_ft()
#crisis_text_counts_ft4 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "crisis_text_counts_ft4.png"), zoom=1)
crisis_text_counts_ft4

crisis_text_counts_ft5 <- temp_all %>% ungroup() %>% filter(crisno %in% c(241:300) ) %>% convert_to_ft() 
#crisis_text_counts_ft5 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "crisis_text_counts_ft5.png"), zoom=1)
crisis_text_counts_ft5

crisis_text_counts_ft6 <- temp_all %>% ungroup() %>% filter(crisno %in% c(301:360) ) %>% convert_to_ft() 
#crisis_text_counts_ft6 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "crisis_text_counts_ft6.png"), zoom=1)
crisis_text_counts_ft6

crisis_text_counts_ft7 <- temp_all %>% ungroup() %>% filter(crisno %in% c(361:420) ) %>% convert_to_ft() 
#crisis_text_counts_ft7 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "crisis_text_counts_ft7.png"), zoom=1)
crisis_text_counts_ft7

crisis_text_counts_ft8 <- temp_all %>% ungroup() %>% filter(crisno %in% c(421:480) ) %>% convert_to_ft() 
#crisis_text_counts_ft8 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "crisis_text_counts_ft8.png"), zoom=1)
crisis_text_counts_ft8
```
...
## Verb Meanings (Glosses) found in the ICB Corpus (top 200) {#app:verbs}
\hphantom{em}
...
if(fromscratch){
  ...
  spacy_initialize(model = "en_core_web_sm") # if this initially doesn't work, try the previous spacy_install command and restart R. 
...
#this is all possible matching glosses per ICBe sentence
...
# can install python packages in R using code chunk above
import torch
...
#Make sure you have a gpu if you want to run this
...
```{r wordnet trees, echo=F, results='hide', message=F, warning=F, eval=T , cache=F}
  ...
  gloss_clean[!condition_glosses_embeddings] <- rownames(glosses_embeddings)[1] #
  ...
  sentences_clean[!condition_sentences_embeddings] <- rownames(sentences_embeddings)[1] #
...
verbs_sentence_wordnet_top_ft <-
        ...
        #fit_to_width(max_width=12) #This is slow
        #flextable::autofit() 
...
## Graphical User Interface {#app:gui}
\hphantom{em}
...
## Coding Aggregation and Filtering {#app:algo1}
\hphantom{em}
...
## Intercoder Agreement {#app:intercoderagreement}
\hphantom{em}
...
```{r, eval=T, echo=F, results='markup', include=T, message=F, cache=F, warning=F,  ft.arraystretch=0.75, fig.width=6, fig.height=6}
  ...
  ggplot2::ggsave(file=paste0(here::here(), '/replication_paper/psrm_draft/p_percent_chose_tag_by_concept.png'), 
...
## Intercoder Agreement by Confidence {#app:agreement_by_confidence}
\hphantom{em}
...
```{r intercoder agreement , eval=T, echo=F, results='markup', include=T, message=F, cache=F, warning=F,  ft.arraystretch=0.75, fig.width=6, fig.height=6}
  ...
  ggplot2::ggsave(file=paste0(here::here(), '/replication_paper/psrm_draft/p_percent_chose_tag_by_confidence.png'), 
...
## Synthetic Narrative Matches Cuban Missile
\hphantom{em}
...
paste0(here::here(), '/replication_data/out/case_study_cuban.Rds')
...
# cuba_recall  %>% saveRDS(paste0(here::here(), '/replication_data/out/case_study_cuban.Rds'))
cuba_recall <- readRDS(paste0(here::here(), '/replication_data/out/case_study_cuban.Rds'))
...
ft_cuba_recall<- cuba_recall %>% 
                            ...
                            #flextable::width( j = 4, width=.3)  %>% 
                            #flextable::width( j = 5, width=.25)  %>% 
                            flextable::width( j = 6:8, width=2.0)  %>% 
                            #flextable::width( j = 6, width=1)  %>%
                            #flextable::width( j = 7, width=1.2)  %>% 
                            #flextable::width( j = 8, width=2)  %>% 
...
ft_cuba_recall %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "ft_cuba_recall.png"), zoom=1) # webshot = "webshot2"
...
## Synthetic Narrative Matches Crimea-Donbas
\hphantom{em}
...
#                                sheet="final") %>% janitor::clean_names()                                
# crimea_recall  %>% saveRDS(paste0(here::here(), '/replication_data/out/case_study_crimea.Rds'))
crimea_recall <- readRDS(paste0(here::here(), '/replication_data/out/case_study_crimea.Rds'))
...
ft_crimea_recall %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "ft_crimea_recall.png"), zoom=1) # webshot = "webshot2"
...
```{r}
rm(ft_cuba_recall, ft_crimea_recall)
gc()
```


# Precision
## Crimea-Donbas Narrative
\hphantom{em}
...
#<!-- remember that sentence 19 broken and cuts off 
...
temp <- ICBe_events_agreed_markdown %>%
        ...
        group_by(crisno, sentence_number_int_aligned) %>%
          filter(!duplicated(value_markdown %>% trimws() )) %>% 
        ungroup() %>%
...
temp <-temp %>%
  ...
  ft_crimea_precision_part1 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "case_study_crimea_precision_part1.png"), zoom=1) # webshot = "webshot2"
  ft_crimea_precision_part1
...
```{r}
  ...
  ft_crimea_precision_part2 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "case_study_crimea_precision_part2.png"), zoom=1) # webshot = "webshot2"
  
ft_crimea_precision_part2
  temp <- NULL
```
...
## Stratified Sentence Examples

```{r Stratified Sentence Examples, eval=T, results='hide', echo=F, include=T, message=F, cache=F}
ICBe_events_agreed <- readRDS( paste0(here::here(), "/replication_data/out/ICBe_V1.1_events_agreed.Rds")) 

temp <- ICBe_events_agreed %>% left_join(ICBe_events_agreed_markdown) %>% dplyr::select(
...
#          stringr::str_replace_all(stringr::fixed("/home/tlscherer/Desktop/ICBEdataset/replication_data/in/flags_small//"), stringr::fixed("")))
...
ft_sample_precision_precision1 <- temp_sample %>% 
                                  ...
                                  flextable::line_spacing( space = 1.0, part = "all") %>% 
                                  padding( padding = 1, part = "all") %>% 
                                  bg( i = which( ((1:22) %% 2 )==1 ) , part = "body", bg = "#EFEFEF") %>% #, j=1:3
                                  ftExtra::colformat_md(j = 5, part="body")

ft_sample_precision_precision1 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "ft_sample_precision_precision1.png"), zoom=1) # 
ft_sample_precision_precision1

ft_sample_precision_precision2 <- temp_sample %>% 
                                  filter(row_number() %in% 23:54) %>%
                                  dplyr::select(name, value, crisno,  ICB=sentence_span_text, ICBe=value_markdown)  %>%
                                  flextable::as_flextable() %>%
                                  flextable::width( j = 1:2, width=0.5)  %>% 
                                  #flextable::width( j = 2, width=0.25)  %>% 
                                  ...
                                  flextable::line_spacing( space = 1.0, part = "all") %>% 
                                  padding( padding = 1, part = "all") %>% 
                                  bg( i = which( ((23:54) %% 2 )==1 ) , part = "body", bg = "#EFEFEF") %>% #, j=1:3
                                  ftExtra::colformat_md(j = 5, part="body")

ft_sample_precision_precision2 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "ft_sample_precision_precision2.png"), zoom=1) # 
ft_sample_precision_precision2

ft_sample_precision_precision3 <- temp_sample %>% 
                                  filter(row_number() %in% 55:90) %>%
                                  dplyr::select(name, value, crisno,  ICB=sentence_span_text, ICBe=value_markdown)  %>%
                                  flextable::as_flextable() %>%
                                  flextable::width( j = 1:2, width=0.5)  %>% 
                                  #flextable::width( j = 2, width=0.25)  %>% 
                                  ...
                                  flextable::line_spacing( space = 1.0, part = "all") %>% 
                                  padding( padding = 1, part = "all") %>% 
                                  bg( i = which( ((55:90) %% 2 )==1 ) , part = "body", bg = "#EFEFEF") %>% #, j=1:3
                                  ftExtra::colformat_md(j = 5, part="body")

ft_sample_precision_precision3 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "ft_sample_precision_precision3.png"), zoom=1) # 
ft_sample_precision_precision3

ft_sample_precision_precision4 <- temp_sample %>% 
                                  filter(row_number() %in% 91:115) %>%
                                  dplyr::select(name, value, crisno,  ICB=sentence_span_text, ICBe=value_markdown)  %>%
                                  flextable::as_flextable() %>%
                                  flextable::width( j = 1:2, width=0.5)  %>% 
                                  #flextable::width( j = 2, width=0.25)  %>% 
                                  ...
                                  flextable::line_spacing( space = 1.0, part = "all") %>% 
                                  padding( padding = 1, part = "all") %>% 
                                  bg( i = which( ((91:115) %% 2 )==1 ) , part = "body", bg = "#EFEFEF") %>% #, j=1:3
                                  ftExtra::colformat_md(j = 5, part="body")

ft_sample_precision_precision4 %>% save_as_image(path=here::here("replication_paper", "psrm_draft", "ft_sample_precision_precision4.png"), zoom=1) # 
ft_sample_precision_precision4
...
```{r}
rm(case_study_crimea_precision_part1.png, case_study_crimea_precision_part2.png, ft_sample_precision_precision1.png, ft_sample_precision_precision2.png, ft_sample_precision_precision3.png, ft_sample_precision_precision4.png)
gc()
```


## Cuban Missiles Crisis Maps
...
saveRDS(cameo_actor_codes,paste0(here::here(), "/replication_paper/data/out/cameo_actor_codes.Rds")
dim(cameo_actor_codes)



replace_emoticons <- function(x){
  x %>% stringr::str_replace_all(stringr::fixed("![](flags_small//"), stringr::fixed("")) %>%
  stringr::str_replace_all(stringr::fixed(".png)"), stringr::fixed("")) %>%
  stringr::str_replace_all(stringr::fixed("Q47488"), stringr::fixed("ICC")) %>%
  stringr::str_replace_all(stringr::fixed("Q1065"), stringr::fixed("\U0001f1fa\U0001f1f3")) %>%
  stringr::str_replace_all(stringr::fixed("Q7184"), stringr::fixed("NATO")) %>%
  stringr::str_replace_all(stringr::fixed("Q1014"), stringr::fixed("\U0001f1f1\U0001f1f7")) %>%
  stringr::str_replace_all(stringr::fixed("Q1027"), stringr::fixed("\U0001f1f2\U0001f1fa")) %>% #Mauritius
  stringr::str_replace_all(stringr::fixed("Q1028"), stringr::fixed("\U0001f1f2\U0001f1e6")) %>%
  stringr::str_replace_all(stringr::fixed("Q1033"), stringr::fixed("\U0001f1f3\U0001f1ec")) %>% #Nigeria
  stringr::str_replace_all(stringr::fixed("Q241"), stringr::fixed("\U0001f1e8\U0001f1fa")) %>%
  stringr::str_replace_all(stringr::fixed("Q159"), stringr::fixed("\U0001f1f7\U0001f1fa")) %>%
  stringr::str_replace_all(stringr::fixed("Q148"), stringr::fixed("\U0001f1e8\U0001f1f3")) %>%
  stringr::str_replace_all(stringr::fixed("Q794"), stringr::fixed("\U0001f1ee\U0001f1f7")) %>%
  stringr::str_replace_all(stringr::fixed("Q458"), stringr::fixed("\U0001f1ea\U0001f1fa")) %>%
  stringr::str_replace_all(stringr::fixed("Q212"), stringr::fixed("\U0001f1fa\U0001f1e6")) %>%
  stringr::str_replace_all(stringr::fixed("Q810"), stringr::fixed("\U0001f1ef\U0001f1f4")) %>% #Jordan
  stringr::str_replace_all(stringr::fixed("Q142"), stringr::fixed("\U0001f1eb\U0001f1f7")) %>%
  stringr::str_replace_all(stringr::fixed("Q183"), stringr::fixed("\U0001f1e9\U0001f1ea")) %>%
  stringr::str_replace_all(stringr::fixed("Q801"), stringr::fixed("\U0001f1ee\U0001f1f1")) %>%
  stringr::str_replace_all(stringr::fixed("Q750"), stringr::fixed("\U0001f1e7\U0001f1f4")) %>%
  stringr::str_replace_all(stringr::fixed("Q298"), stringr::fixed("\U0001f1e8\U0001f1f1")) %>%
  stringr::str_replace_all(stringr::fixed("Q804"), stringr::fixed("\U0001f1f5\U0001f1e6")) %>%
  stringr::str_replace_all(stringr::fixed("Q155"), stringr::fixed("\U0001f1e7\U0001f1f7")) %>%
  stringr::str_replace_all(stringr::fixed("Q145"), stringr::fixed("\U0001f1ec\U0001f1e7")) %>%
  stringr::str_replace_all(stringr::fixed("Q783"), stringr::fixed("\U0001f1ed\U0001f1f3")) %>%
  stringr::str_replace_all(stringr::fixed("Q252"), stringr::fixed("\U0001f1ee\U0001f1e9")) %>%
  stringr::str_replace_all(stringr::fixed("Q912"), stringr::fixed("\U0001f1f2\U0001f1f1")) %>% #Mali
  stringr::str_replace_all(stringr::fixed("Q929"), stringr::fixed("\U0001f1e8\U0001f1eb")) %>% #Central African Republic
  stringr::str_replace_all(stringr::fixed("Q184"), stringr::fixed("\U0001f1e7\U0001f1fe")) %>% #Belarus
  stringr::str_replace_all(stringr::fixed("Q796"), stringr::fixed("\U0001f1ee\U0001f1f6")) %>% #Iraq 	
  stringr::str_replace_all(stringr::fixed("Q423"), stringr::fixed("\U0001f1f0\U0001f1f5")) %>% #North Korea
  stringr::str_replace_all(stringr::fixed("Q843"), stringr::fixed("\U0001f1f5\U0001f1f0")) %>%
  stringr::str_replace_all(stringr::fixed("Q668"), stringr::fixed("\U0001f1ee\U0001f1f3")) %>%
  stringr::str_replace_all(stringr::fixed("Q262"), stringr::fixed("\U0001f1e9\U0001f1ff")) %>%
  stringr::str_replace_all(stringr::fixed("Q766"), stringr::fixed("\U0001f1ef\U0001f1f2")) %>%
  stringr::str_replace_all(stringr::fixed("Q408"), stringr::fixed("\U0001f1e6\U0001f1fa")) %>% #Australia
  stringr::str_replace_all(stringr::fixed("Q858"), stringr::fixed("\U0001f1f8\U0001f1fe")) %>% #Syria
  stringr::str_replace_all(stringr::fixed("Q958"), stringr::fixed("\U0001f1f8\U0001f1f8")) %>% #South Sudan
  stringr::str_replace_all(stringr::fixed("Q211"), stringr::fixed("\U0001f1f1\U0001f1fb")) %>% #Latvia
  stringr::str_replace_all(stringr::fixed("Q833"), stringr::fixed("\U0001f1f2\U0001f1fe")) %>% #Malaysia
  stringr::str_replace_all(stringr::fixed("Q217"), stringr::fixed("\U0001f1f2\U0001f1e9")) %>% #Moldova
  stringr::str_replace_all(stringr::fixed("Q822"), stringr::fixed("\U0001f1f1\U0001f1e7")) %>% #Lebanon
  stringr::str_replace_all(stringr::fixed("Q889"), stringr::fixed("\U0001f1e6\U0001f1eb")) %>% #Afghanistan
  stringr::str_replace_all(stringr::fixed("Q43"), stringr::fixed("\U0001f1f9\U0001f1f7")) %>%
  stringr::str_replace_all(stringr::fixed("Q31"), stringr::fixed("\U0001f1e7\U0001f1ea")) %>% #Belgium
  stringr::str_replace_all(stringr::fixed("Q41"), stringr::fixed("\U0001f1ed\U0001f1f3")) %>%
  stringr::str_replace_all(stringr::fixed("Q38"), stringr::fixed("\U0001f1ee\U0001f1f9")) %>% #Italy
  stringr::str_replace_all(stringr::fixed("Q39"), stringr::fixed("\U0001f1e8\U0001f1ed")) %>%
  stringr::str_replace_all(stringr::fixed("Q16"), stringr::fixed("\U0001f1e8\U0001f1e6")) %>% #Canada
  stringr::str_replace_all(stringr::fixed("Q37"), stringr::fixed("\U0001f1f1\U0001f1f9")) %>% #Lithuania
  stringr::str_replace_all(stringr::fixed("Q36"), stringr::fixed("\U0001f1f5\U0001f1f1")) %>%
  stringr::str_replace_all(stringr::fixed("Q29"), stringr::fixed("\U0001f1ea\U0001f1f8")) %>%
  stringr::str_replace_all(stringr::fixed("Q17"), stringr::fixed("\U0001f1ef\U0001f1f5")) %>%
  stringr::str_replace_all(stringr::fixed("Q20"), stringr::fixed("\U0001f1f3\U0001f1f4")) %>% #Norway
  stringr::str_replace_all(stringr::fixed("Q30"), stringr::fixed("\U0001f1fA\U0001f1f8")) 
}
...
icb2v14_clean <- icb2v14 %>% 
                 left_join(icb_df %>% dplyr::select(cracid=icb_code, actor_label=icb_name ) %>% mutate(cracid=cracid %>% as.numeric()) ) %>%  
...
icb_crisismaps <- function(thiscrisis=196, prune_rare=T){
    ...
    print(thiscrisis)
    save_icb_file <- paste0(here::here(), "/replication_paper/psrm_draft/p_icb_metro_plot_",thiscrisis,".Rds")
    ...
    ideal_ordering <- hc$labels[hc$order] 
    ...
    actor_colors = c(brewer.pal(n=11, "Dark2"), brewer.pal(n=11, "Set1"))[1:length(actors)] #pick a bewer pallet with lots of values and then assign it to specific values
...
c(196) %>% sapply(FUN=function(x) icb_crisismaps(x)) #This just runs every time no from scratch because the files are small

c(471) %>% sapply(FUN=function(x) icb_crisismaps(x)) #This just runs every time no from scratch because the files are small
...
dyadic_mid_clean <- dyadic_mid %>% distinct() %>%
                      left_join(actor_translator %>% dplyr::select(statea=COWcode, actor_1=QCode_name, actor_1_qcode=QCode)) %>% # we add color here
...
mids_crisismaps <- function(thiscrisis=196, prune_rare=T){
  ...
  save_mids_file <- paste0(here::here(), "/replication_paper/psrm_draft/p_mids_metro_plot_",thiscrisis,".Rds")
...
```{r}
rm(dyadic_mid, dyadic_mid_clean)
gc()
```

```{r phoenix data loads, eval=F, results='hide', echo=F, include=T, message=F, cache=F, warning=F,}
...
library(stringr)
  ...
  phoenix_clean_471 %>% write_tsv(paste0(here::here(), "/replication_data/out/phoenix_clean_471.tsv"))
  ...
  phoenix_clean_196 %>% write_tsv(paste0(here::here(), "/replication_data/out/phoenix_clean_196.tsv"))
...
phoenix_barmaps <- function(thiscrisis=196, phoenix_subset="phoenix_clean_196.tsv", topactor=F){

    print(thiscrisis)
    plot_title <- crisno_name_years %>% filter(crisno==thiscrisis) %>% mutate(title="Crisis #" %>% paste0(crisno, " ", crisname, " (", yrtrig, "-",yrterm,")")) %>% pull(title)
    save_file_penta <- paste0(here::here(), "/replication_paper/psrm_draft/p_phoenix_plot_penta_",thiscrisis,".Rds")
    save_file_lv0 <- paste0(here::here(), "/replication_paper/psrm_draft/p_phoenix_plot_lv0_",thiscrisis,".Rds")
    ...
    phoenix <- read_tsv(paste0(here::here(), "/replication_data/out/phoenix_clean_196.tsv"))
    
    phoenix_clean_subset <- phoenix %>% 
    ...
    names_to_qcodes <- actor_translator %>% dplyr::select(QCode, contains("name")) %>%
                       ...
                       mutate(markdown = glue::glue("![](flags_small//{QCode}.png)") ) 
    ...
    phoenix_clean_subset_top10 <- phoenix_clean_subset_top10 %>%
                                   ...
                                   dplyr::mutate(dyad_markdown = dyad_markdown %>% replace_emoticons)
    ...
    names_to_qcodes <- actor_translator %>% dplyr::select(QCode, contains("name")) %>%
                       pivot_longer(-c(QCode)) %>% na.omit() %>% dplyr::select(-name) %>% distinct() %>%
                       mutate(markdown = glue::glue("![](flags_small//{QCode}.png)") )  # since direct flags are not working
    names_to_qcodes <- bind_rows(names_to_qcodes, names_to_qcodes %>% mutate(value= value %>% tolower() )) %>% distinct() %>%
                       arrange(desc(nchar(value))) #sort by length so you never accidentally replace a subset
    ...
    phoenix_clean_subset_top10 <- phoenix_clean_subset_top10 %>%
                                   ...
                                   mutate(dyad_markdown = dyad_markdown %>% str_replace_all("\\(tertiary role code\\)","")) %>%
                                   mutate(dyad_markdown = dyad_markdown %>% str_replace_all("unidentified armed force"," unknown force")) %>%
                                   dplyr::mutate(dyad_markdown = dyad_markdown %>% replace_emoticons) # since direct flags are not working
...
phoenix_barmaps(thiscrisis=196, phoenix_subset="phoenix_clean_196.tsv" , topactor=F)
phoenix_barmaps(thiscrisis=471, phoenix_subset="phoenix_clean_471.tsv" , topactor=F)
```

```{r Terrierr Data Loads, eval=F, results='hide', echo=F, include=T, message=F, cache=F, warning=F}
...
terrier_barmaps <- function(thiscrisis=471, topactor=F){
    ...
    print(thiscrisis)
    plot_title <- crisno_name_years %>% filter(crisno==thiscrisis) %>% mutate(title="Crisis #" %>% paste0(crisno, " ", crisname, " (", yrtrig, "-",yrterm,")")) %>% pull(title)
    save_file_penta <- paste0(here::here(), "/replication_paper/psrm_draft/p_terrier_plot_penta_",thiscrisis,".Rds")
    save_file_lv0 <- paste0(here::here(), "/replication_paper/psrm_draft/p_terrier_plot_lv0_",thiscrisis,".Rds")
    ...
    #                 mutate(story_date=paste(month, day, year, sep="/")  %>% lubridate::mdy() ) %>%
    #                 filter(story_date>=(date_start-lubridate::years(1)) & story_date<=(date_end+lubridate::years(1))) 
    #                terrier_clean_subset  %>%   arrow::write_parquet("terrier_clean.parquet")

    terrier_clean_subset <- terrier_clean_subset %>%   
  ...
  terrier_clean <-  arrow::read_parquet("terrier_clean.parquet") 
  dim(terrier_clean) #28,438,029 #39,322,909 #7,847,463
  
  c(471) %>% sapply(FUN=function(x) terrier_barmaps(x))
  # c(196) %>% sapply(FUN=function(x) terrier_barmaps(x))
}
...
icews_barmaps <- function(thiscrisis=471, topactor=F){
    ...
    print(thiscrisis)
    plot_title <- crisno_name_years %>% filter(crisno==thiscrisis) %>% mutate(title="Crisis #" %>% paste0(crisno, " ", crisname, " (", yrtrig, "-",yrterm,")")) %>% pull(title)
    save_file_penta <- paste0(here::here(), "/replication_paper/psrm_draft/p_icews_plot_penta_",thiscrisis,".Rds")
    save_file_lv0 <- paste0(here::here(), "/replication_paper/psrm_draft/p_icews_plot_lv0_",thiscrisis,".Rds")
    ...
    actors <- icb_events_long %>% filter(crisno %in% thiscrisis) %>% filter(varname_normalized %>% str_detect('actor')) %>% 
              dplyr::select(value_normalized,value_qcode) %>% distinct()
    #library(re2r) #didn't speed it up and enabling parallel was actually slower,
    library(tictoc)
    tictoc::tic() 
...
# if(fromscratch){
  ...
  icews_clean <-  arrow::read_parquet("icews_clean.parquet")
...
## Dyadic ICB
save_file=paste0(here::here(), '/replication_paper/psrm_draft/p_icb_metro_plot_196.Rds')
...
## MIDs
library(ggplot2)
save_file=paste0(here::here(), '/replication_paper/psrm_draft/p_mids_metro_plot_196.Rds') 
p_metro_plot <- readRDS(save_file)
actor_colors <- p_metro_plot$actor_colors
...
#ggsave(file=paste0(here::here(), '/replication_paper/psrm_draft/p_mids_metro_plot_196.png'),
#       plot = p_metro_plot ,
#       width=12, height=14)
...
#ICBe  196
library(ggplot2)
save_file=paste0(here::here(), '/replication_paper/psrm_draft/p_phoenix_plot_penta_196.Rds')
...
#ggsave(file=paste0(here::here(), '/replication_paper/psrm_draft/p_phoenix_metro_plot_196.png'),
#       plot = p_metro_plot ,
#       width=12, height=14)
...
save_plot(file=paste0(here::here(), '/replication_paper/psrm_draft/p_196_combined.png'),
...
```{r, eval=T, results='hide', echo=F, include=T, message=F, cache=F, warning=F}
  p_phoenix_plot_lv0_196 <- readRDS(here::here("replication_paper", "psrm_draft", "p_phoenix_plot_lv0_196.Rds"))
  save_plot(file=paste0(here::here(), '/replication_paper/psrm_draft/p_phoenix_plot_lv0_196.png'),
...
## Crimea-Donbas Crisis Maps 

<!-- p_471_icbe.png is created in ICBe_manuscript -->
...
## MIDs
library(ggplot2)
save_file=paste0(here::here(), '/replication_paper/psrm_draft/p_mids_metro_plot_471.Rds') 
p_metro_plot <- readRDS(save_file)
actor_colors <- p_metro_plot$actor_colors
subtitle="Dyadic Militarized Interstate Disputes (MIDs)"
caption=paste0("Source: CrisisEvents.org (Douglass et al. 2021)
...
#ICBe  196
library(ggplot2)
save_file=paste0(here::here(), '/replication_paper/psrm_draft/p_phoenix_plot_penta_471.Rds')
subtitle="Phoenix Dyadic Events"
caption=paste0("Data: Cline Center Historical Phoenix Event Data (Althaus et al. 2017)
             ", p_metro_plot$labels$caption) #tag original to bottom
...
save_file=paste0(here::here(), '/replication_paper/psrm_draft/p_terrier_plot_penta_471.Rds')
...
save_file=paste0(here::here(), '/replication_paper/psrm_draft/p_icews_plot_penta_471.Rds')
p_471_icews <- readRDS(save_file) 



save_plot(file=paste0(here::here(), '/replication_paper/psrm_draft/p_471_mids.png'),
          p_471_mids, base_height = 18, base_width = 13.5)
...
save_plot(file=paste0(here::here(), '/replication_paper/psrm_draft/p_471_combined.png'),
          p_471_combined, base_height = 14, base_width = 23)

```

<!-- \begin{figure}[H] -->
<!-- \caption{Crisis Map: Cuban Missile Crisis \label{fig:p_471_mids.png}} -->
<!-- \centering{\includegraphics[width=17cm]{./p_471_mids.png}} -->
<!-- \end{figure} -->
...
p_phoenix_plot_lv0_471 <- readRDS(here::here("replication_paper", "psrm_draft", "p_phoenix_plot_lv0_471.Rds"))
save_plot(file=paste0(here::here(), '/replication_paper/psrm_draft/p_phoenix_plot_lv0_471.png'),
...
```{r, eval=T, results='hide', echo=F, include=T, message=F, cache=F, warning=F}
  p_terrier_plot_lv0_471 <- readRDS(here::here("replication_paper", "psrm_draft", "p_terrier_plot_lv0_471.Rds"))
  save_plot(file=paste0(here::here(), '/replication_paper/psrm_draft/p_terrier_plot_lv0_471.png'),
...
```{r, eval=T, results='hide', echo=F, include=T, message=F, cache=F, warning=F}
  ...
  p_icews_plot_lv0_471 <- readRDS(here::here("replication_paper", "psrm_draft", "p_icews_plot_lv0_471.Rds"))
  save_plot(file=paste0(here::here(), '/replication_paper/psrm_draft/p_icews_plot_lv0_471.png'),
