...
ft_cuba_precision %>% save_as_image(path="/mnt/8tb_a/rwd_github_private/ICBEdataset/replication_paper/pnas_draft/case_study_cuban_precision.png", zoom=1) # webshot = "webshot2"
temp <- NULL
...
\centering{\includegraphics[ height=22cm]{"/mnt/8tb_a/rwd_github_private/ICBEdataset/replication_paper/pnas_draft/case_study_cuban_precision.png"}}
\end{figure}
\clearpage
\twocolumn
...
litreview_ft_pnas <- lit_review_alt %>%
      ...
      #        #"ICB Corpus"="Corpus",
      #        "ICBe"="Ours",
      #        "Phoenix"="CAMEO Dictionary\nSystems",
      #        "Terrier"="CAMEO Dictionary\nSystems",
      #        "ICEWs"="CAMEO Dictionary\nSystems",
      #        "MIDs Events"="Disputes",
      #        "UCDP-GED"="Violence",
      #        "ICB"="Crises",
      #        "COW"="Wars",
      #        "MIDs"="Disputes",
...
ft   %>% 
    #flextable::flextable_to_rmd()
    save_as_image(path="/mnt/8tb_a/rwd_github_private/ICBEdataset/replication_paper/pnas_draft/case_study_cuban_recall.png", zoom=1, webshot = "webshot2") # webshot = "webshot2"
...
ft   %>% 
    #flextable::flextable_to_rmd()
    save_as_image(path="/mnt/8tb_a/rwd_github_private/ICBEdataset/replication_paper/pnas_draft/case_study_crimea_recall.png",
...
ft   %>% 
    #flextable::flextable_to_rmd()
    save_as_image(path="/mnt/8tb_a/rwd_github_private/ICBEdataset/replication_paper/pnas_draft/recall_cuban_and_crimea.png", zoom=1, webshot = "webshot2") # webshot = "webshot2"
...
ggsave(file=paste0(here::here(), '/replication_paper/figures/p_recall.png'),
       plot = p_recall ,
       width=5, height=3.0)

library(magick)
tiger <- image_read("/mnt/8tb_a/rwd_github_private/ICBEdataset/replication_paper/pnas_draft/recall_cuban_and_crimea.png")
tiger2 <- image_read(paste0(here::here(), '/replication_paper/figures/p_recall.png'))

combined <- image_composite(tiger, image_scale(tiger2, "x290"), offset = "+475+650")
image_write(combined, "/mnt/8tb_a/rwd_github_private/ICBEdataset/replication_paper/pnas_draft/recall_cuban_and_crimea_andcounts.png")
...
\centering{\includegraphics[height=18cm]{/mnt/8tb_a/rwd_github_private/ICBEdataset/replication_paper/pnas_draft/recall_cuban_and_crimea_andcounts.png}}
...
p_196_icbe <- icbe_crisismaps(thiscrisis=196) #readRDS(file=paste0(here::here(), '/replication_paper/figures/crisismaps/p_metro_plot_471.Rds')) +
    labs(
      #title = plot_title ,
      #subtitle = subtitle ,
      caption = NULL
...
p_471_icbe <- icbe_crisismaps(thiscrisis=471) #readRDS(file=paste0(here::here(), '/replication_paper/figures/crisismaps/p_metro_plot_471.Rds')) +
    labs(
      #title = plot_title ,
      #subtitle = subtitle ,
      caption = NULL
...
save_plot(file=paste0(here::here(), '/replication_paper/figures/p_precision_combined.png'),
          p_precision_combined, base_height = 14, base_width = 18)
...
\centering{\includegraphics[width=24cm]{../figures/p_precision_combined.png}}
\end{sidewaysfigure}
\clearpage
\twocolumn
...
p_semantic_embeddings <-
      ...
      geom_label(data=value_means,
                 aes(x=UMAP2,y=UMAP1, fill = value %>% as.factor(), label=value, size=50), size = 2.5, inherit.aes=F, 
...
#This is unreasonably slow because it's large and in tidy. Couldn't do it every time if we wanted to.
value_highdim <- ICBe_events_agreed_markdown_embedded %>%
                 dplyr::select(starts_with("V",  ignore.case = F), contains("escalate"),contains("coop"),contains("sayintkind"), 
                               contains("thinkkind"), crisno, sentence_number_int_aligned) %>% 
                 pivot_longer(cols=-c(starts_with("V",  ignore.case = F), crisno, sentence_number_int_aligned) ) %>%
                 filter(!is.na(value) & value!='') %>% #40,898
                 filter(!value %>% str_detect("sentence|^[0-9]")) %>% 
                 mutate(value = value %>% str_split(";")) %>% 
                 unnest(value) %>% 
                 mutate(value = value %>% str_trim())   %>% #
                 ...
                 ungroup() %>%
                 arrange(n, value) %>%
                 filter(!duplicated( paste(crisno, sentence_number_int_aligned)) ) %>% #only take one per sentence to break mechanically induced correlations
                 filter(n>20) 

dim(value_highdim)

value_highdim_medians <- value_highdim %>% 
                           group_by(color, value) %>%
                           dplyr::summarise_all(median) 
  
value_normalized_colors <- value_highdim_medians$color 
names(value_normalized_colors) = value_highdim_medians$value

m <- value_highdim_medians %>% dplyr::select(starts_with("V",  ignore.case = F)) %>% as.matrix() 
rownames(m) <- value_highdim_medians$value
d <- m %>% dist()
...
p_dendro <- ggplot() +
            ...
            geom_text(data = label(hcdata) %>% mutate(color=value_highdim_medians$color ), 
                      aes(x = x, y = y, label = label, hjust = 0, color=color[hc$order]), 
                      size = 3
            ) +
            coord_flip() +
...
ggsave(file=paste0(here::here(), '/replication_paper/figures/p_semantic_embeddings_dendro.png'), plot = p_combined, width=12, height=10)
...
icews_clean_471 <- read_tsv("/mnt/8tb_a/rwd_github_private/ICBEdataset/replication_paper/data/out/icews_clean_471.tsv")
dim(icews_clean_471) #270,642
icews_clean_471_small <- read_tsv("/mnt/8tb_a/rwd_github_private/ICBEdataset/replication_paper/data/out/icews_clean_471_small.tsv")

icews_clean_subset_top10_day <- read_tsv("/mnt/8tb_a/rwd_github_private/ICBEdataset/replication_paper/data/out/icews_clean_471_lowest.tsv")
...
save_plot(file=paste0(here::here(),
                      '/replication_paper/figures/p_precision_icews.png'),
...
\onecolumn
\begin{figure}[H]
\caption{ ICEWs Counts of Events per Day by Type during the Crimea-Donbas Crisis \label{fig:p_precision_icews}}
\centering{\includegraphics[width=17cm]{../figures/p_precision_icews.png}}
\textit{Notes:Unit of analysis is the Dyad-Day. Edges <-> indicates undirected dyad and -> indicates directed dyad. Top 10 most active dyads per category shown. Red text shows events from the synthetic narrative relative to that event category. Event types are grouped by hand along similar themes. Blue bars indicate an event recorded by ICEWs for that dyad on that day. }
\end{figure}
\clearpage
\twocolumn
