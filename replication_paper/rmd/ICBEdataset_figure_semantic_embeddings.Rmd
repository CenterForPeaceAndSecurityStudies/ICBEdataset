...
codings_long <- readRDS(file=paste0(here::here(), '/data_out/codings_long.Rds'))
codings_long_agreement <- readRDS(file=paste0(here::here(), '/data_out/codings_long_agreement.Rds'))
...
library(umapr)
embedding <- umap(sentences_embeddings  )
...
codings_long_agreed_embeded %>% saveRDS(paste0(here::here(), '/replication_data/temp/codings_long_agreed_embeded.Rds'))

codings_wide_agreed <- readRDS(paste0(here::here(), '/replication_data/out/codings_wide_agreed.Rds'))
codings_wide_agreed_embeded <- codings_wide_agreed %>% ungroup() %>% left_join(sentences_embeded)
...
codings_wide_agreed_embeded %>% saveRDS(paste0(here::here(), '/replication_data/temp/codings_wide_agreed_embeded.Rds'))
...
value_means <- codings_long_agreed_embeded %>%
      ...
      group_by(variable_normalized, value_normalized) %>% 
      summarise_all(median) %>%
      group_by(variable_normalized) %>%
          mutate(variable_normalized_count=  min(n(),4)  ) %>%
      ungroup()
...
brewer_pal_info_df <- brewer.pal.info %>% add_rownames( var = "rowname") %>% arrange()
brewer_list <- list() 
for(i in c(11, 1:nrow(brewer_pal_info_df))){
...
value_normalized=codings_long_agreed_embeded_subset$value_normalized %>% unique() %>% as.factor()
value_normalized_colors = posible_colors[1:length(value_normalized)] #ok so pick a bewer pallet with lots of values and then assign it to specific values
names(value_normalized_colors) = levels(value_normalized)
...
p_semantic_embeddings <-
      ...
      ggplot(aes(x=UMAP1,y=UMAP2, color=value_normalized )) +
        ...
        geom_label(data=value_means,
                   aes(x=UMAP1,y=UMAP2, fill = value_normalized, label=value_normalized, size=50),inherit.aes=F, # #variable_normalized_count
...
p_semantic_embeddings   %>% saveRDS(paste0(here::here(), '/paper_replication/figures/p_semantic_embeddings.Rds'))
p_semantic_embeddings
