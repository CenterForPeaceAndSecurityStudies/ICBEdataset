...
avg_coders_per_crisis <- icb_wide_clean %>% dplyr::filter(crisno != 196) %>% dplyr::mutate(cris_coder = paste(crisno, email_crc32)) %>% dplyr::select(crisno, email_crc32) %>% unique() %>% dplyr::count(crisno) %>% dplyr::select(n) %>% lapply(mean)
...
avg_events_per_coder_crisis <- icb_wide_clean %>% dplyr::filter(crisno != 196) %>% dplyr::mutate(cris_coder = paste(crisno, email_crc32)) %>% dplyr::count(cris_coder) %>% dplyr::select(n) %>% lapply(mean)
