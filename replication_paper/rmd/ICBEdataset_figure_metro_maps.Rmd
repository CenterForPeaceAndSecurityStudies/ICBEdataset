...
icb_crises <- read.csv(paste0(here::here(), "/replication_data/in/icb1v14.csv")) 
...
codings_long <- readRDS( paste0(here::here(), "/replication_data/temp/codings_long.Rds")) 
codings_long_agreement <- readRDS( paste0(here::here(), "/replication_data/temp/codings_long_agreement.Rds")) 
codings_long_agreed <- readRDS( paste0(here::here(), "/replication_data/out/codings_long_agreed.Rds")) 
codings_wide_agreed <- readRDS( paste0(here::here(), "/replication_data/out/codings_wide_agreed.Rds")) 
...
icbe_metro_plots <- function(crisis=196, prune_rare=T){
  ...
  save_file <- paste0(here::here(), "/paper/figures/metro_plots/p_metro_plot_",crisis,".Rds")
  ...
  units <-   codings_wide_agreed_subset %>% dplyr::select(sent=final_event_ordering, do_actor_a, interact_units) %>% na.omit()  %>% 
              ...
              mutate(do_actor_a = strsplit(as.character(do_actor_a), ";")) %>% unnest(do_actor_a) %>%
              mutate(IMAGE=paste0(here::here, '/paper/figures/metro_plots/symbols/',interact_units,'.png')) %>%
...
codings_wide_agreed$crisno %>% unique() %>% sapply(FUN=function(x) icbe_metro_plots(x))
...
icb_sentence_tables <- function(crisis=196){
  ...
  save_file <- paste0(here::here(), "/paper/tables/sentence_tables/ft_sentence_table_",crisis,".Rds")
...
codings_wide_agreed$crisno %>% unique() %>% sapply(FUN=function(x) icb_sentence_tables(x))
...
dyadic_mid <- read_csv(paste0(here::here(), "/data_other_datasets/mids/Dyadic MIDs 4.02/dyadic_mid_4.02.csv"))
...
mids_metro_plots <- function(crisis=196, prune_rare=T){
  ...
  #crisis=426
  plot_title <- crisno_name_years %>% filter(crisno==crisis) %>% mutate(title="Crisis #" %>% paste0(crisno, " ", crisname, " (", yrtrig, "-",yrterm,")")) %>% pull(title)
  save_mids_file <- paste0(here::here(), "/paper/figures/metro_plots/p_mids_metro_plot_",crisis,".Rds")
...
codings_wide_agreed$crisno %>% unique() %>% sapply(FUN=function(x) mids_metro_plots(x))
...
midip <- read_csv(paste0(here::here(), '/data_other_datasets/mids_incidents/MIDIP_5.01.csv'))

midii <- read_csv(paste0(here::here(), '/data_other_datasets/mids_incidents/MIDI_5.01.csv'))
...
mids_incidents_metro_plots <- function(crisis=196, prune_rare=T){
  ...
  print(crisis)
  #crisis=426
  save_mids_incidents_file <- paste0(here::here(), "/paper/figures/metro_plots/p_mids_incidents_metro_plot_",crisis,".Rds")
...
icb_crises %>% filter(yrtrig>=1990) %>% pull(crisno) %>% unique()  %>% sapply(FUN=function(x) mids_incidents_metro_plots(x))
...
icbdy_20 <- read_tsv(here::here(), "/data_other_datasets/icb_dyad/icbdy_20.dat")
...
icb2v14 <- read_csv(here::here(), "/data_other_datasets/icb/icb2v14.csv")
...
icb_metro_plots <- function(crisis=196, prune_rare=T){
    ...
    print(crisis)
    save_icb_file <- paste0(here::here(), "/paper/figures/metro_plots/p_icb_metro_plot_",crisis,".Rds")
...
codings_wide_agreed$crisno %>% unique() %>% sapply(FUN=function(x) icb_metro_plots(x))
...
ged211 <- read_csv(here::here(), "/data_other_datasets/ucdp_ged/ged211.csv")
...
ucdp_ged_metro_plots <- function(crisis=466, prune_rare=T){
    ...
    print(crisis)
    #crisis=426
    save_ucdp_incidents_file <- paste0(here::here(), "/paper/figures/metro_plots/p_ucdp_metro_plot_",crisis,".Rds")
...
icb_crises %>% filter(yrtrig>=1989) %>% pull(crisno) %>% unique()  %>% sapply(FUN=function(x) ucdp_ged_metro_plots(x))
...
CAMEO_eventcodes <- read_tsv(here::here(), "/data_other_datasets/icews/CAMEO.eventcodes.txt", col_types='cc') %>% janitor::clean_names()
...
#CAMEO_eventcodes <- read_tsv(paste0(here::here(), "/data_temp/CAMEO.eventcodes.txt")) %>% janitor::clean_names()
...
files <- list.files(path = paste0(here::here(), "/data_other_datasets/icews/dataverse_files (1)/"), pattern = ".tab$", full.names = T)
icews <- bind_rows(lapply(files, FUN=function(x) read_tsv(x) )  ) %>% janitor::clean_names()
dim(icews)

CAMEO_eventcodes <- read_tsv(paste0(here::here(),"/data_other_datasets/icews/CAMEO.eventcodes.txt"), col_types='cc') %>% janitor::clean_names()
...
icews_clean <- icews %>% 
               left_join(CAMEO_eventcodes %>% mutate(cameoeventcode=cameoeventcode) %>%  #You have 
...
icews_metro_plots <- function(crisis=426, topactor=F, exclude='', save_icews_file=NULL){
  ...
  #save_icews_file = paste0(here::here(), "/paper/figures/p_icews_metro_plot_",crisis,"_paper.Rds")
  plot_title <- crisno_name_years %>% filter(crisno==crisis) %>% mutate(title="Crisis #" %>% paste0(crisno, " ", crisname, " (", yrtrig, "-",yrterm,")")) %>% pull(title)
  if(is.null(savefile)){
    save_icews_file <- paste0(here::here(), "/paper/figures/metro_plots/p_icews_metro_plot_",crisis,".Rds")
  }
  ...
  p_icews_metro_map  %>% saveRDS(save_icews_file)

}

icb_crises %>% filter(yrtrig>=1995) %>% pull(crisno) %>% unique()  %>% sapply(FUN=function(x) icews_metro_plots(x))
...
a <- read_csv(paste0(here::here, '/data_other_datasets/phoenix/PhoenixBLN-NYT_1980-2018.csv', col_types = 'ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc'))
b  <- read_csv(paste0(here::here, '/data_other_datasets/phoenix/PhoenixBLN-SWB_1979-2019.csv', col_types = 'ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc'))
c  <- read_csv(paste0(here::here, '/data_other_datasets/phoenix/PhoenixFBIS_1995-2004.csv', col_types = 'ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc'))
d  <- read_csv(paste0(here::here, '/data_other_datasets/phoenix/PhoenixNYT_1945-2005.csv', col_types = 'ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc'))
e <- read_csv(paste0(here::here, '/data_other_datasets/phoenix/PhoenixWSJ_1945-2006.csv', col_types = 'ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc'))

phoenix <- bind_rows(a, b, c, d , e)
...
phoenix_clean <- phoenix %>% 
                 ...
                 left_join(CAMEO_eventcodes %>% dplyr::select(code=cameoeventcode     , cameo_event_label=eventdescription) ) %>%
  
                 left_join(CAMEO_eventcodes %>% dplyr::select(root_code=cameoeventcode, cameo_event_root_label=eventdescription) %>% filter(nchar(root_code)==2)) %>%
...
phoenix_metro_plots <- function(crisis=196, topactor=F){
  ...
  #crisis=426
  plot_title <- crisno_name_years %>% filter(crisno==crisis) %>% mutate(title="Crisis #" %>% paste0(crisno, " ", crisname, " (", yrtrig, "-",yrterm,")")) %>% pull(title)
  save_phoenix_file <- paste0(here::here(), "/paper/figures/metro_plots/p_phoenix_metro_plot_",crisis,".Rds")
...
codings_wide_agreed$crisno %>% unique() %>% sapply(FUN=function(x) phoenix_metro_plots(x))
...
files <- list.files(path = paste0(here::here(), "/data_other_datasets/terrier/largegeolocateddata/"), pattern = ".tsv$", full.names = T)
...
CAMEO_eventcodes <- read_tsv(paste0(here::here(), "/data_other_datasets/terrier/CAMEO.eventcodes.txt"), col_types='cc' ) %>% janitor::clean_names()
...
terrier_clean <- terrier %>% 
    ...
    distinct() %>% 
     ...
     left_join(CAMEO_eventcodes %>% mutate(cameoeventcode=cameoeventcode %>% as.numeric(cameoeventcode)) %>%
                 dplyr::select(cameo_code=cameoeventcode, cameo_label= eventdescription)  ) %>%
...
terrier_metro_plots <- function(crisis=196, topactor=F){
  ...
  #crisis=426
  plot_title <- crisno_name_years %>% filter(crisno==crisis) %>% mutate(title="Crisis #" %>% paste0(crisno, " ", crisname, " (", yrtrig, "-",yrterm,")")) %>% pull(title)
  save_terrier_file <- paste0(here::here(), "/paper/figures/metro_plots/p_terrier_metro_plot_",crisis,".Rds")
...
icb_crises %>% filter(yrtrig>=1977) %>% pull(crisno) %>% unique()  %>% sapply(FUN=function(x) terrier_metro_plots(x))
