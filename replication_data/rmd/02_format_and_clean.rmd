...
dim(unique(icb_wide_aligned_raw))
```
...
Drop other instances where coder enters the same information twice

```{r}

# YUGOSLAVIA I: CROATIA-SLOVENIA1 and YUGOSLAVIA I: CROATIA-SLOVENIA2 overlap for one coder, otherwise rows are identical

# z1 <- icb_wide_aligned %>% dplyr::filter(email_id == "expert1" & input_crisis=="YUGOSLAVIA I: CROATIA-SLOVENIA1" & sentence_number_int_aligned %in% c(37:50))
# z2 <- icb_wide_aligned %>% dplyr::filter(email_id == "expert1" & input_crisis=="YUGOSLAVIA I: CROATIA-SLOVENIA2" & sentence_number_int_aligned %in% c(37:50))
# z1==z2

icb_wide_aligned <- icb_wide_aligned %>% dplyr::filter(!(email_id == "expert1" & input_crisis=="YUGOSLAVIA I: CROATIA-SLOVENIA2" & sentence_number_int_aligned %in% c(37:50)))

# one coders entry for SAND WALL appear in duplicate
icb_wide_aligned <- icb_wide_aligned %>% dplyr::mutate(duplicate = duplicated(icb_wide_aligned))
icb_wide_aligned <- icb_wide_aligned %>% dplyr::filter(!(email_id == "undergrad49addaea" & input_crisis=="SAND WALL" & duplicate==TRUE)) %>% dplyr::select(-duplicate)
...
icb_wide_aligned %>% dplyr::filter(icb_wide_aligned %>% dplyr::select(-input_crisis) %>% duplicated())
```
...
```
```{r}
# Check that each entry has only one event_type entry (should be all false)
icb_long %>% dplyr::filter(variable=="event_type") %>% dplyr::select(email_crisis_sent_event) %>% duplicated() %>% table()
```
...
condition_location_merge_directory <- icb_wide_aligned %>% 
    dplyr::mutate(crisno3 = stringr::str_pad(crisno, 3, pad = "0"), 
                  sentence_number_int_aligned3 = stringr::str_pad(sentence_number_int_aligned, 3, pad = "0")) %>% 
  dplyr::mutate(email_crisis_sent_event = paste(email_id,crisno3,sentence_number_int_aligned3,event_number_int, sep="_")) %>%
  dplyr::select(-crisno3, -sentence_number_int_aligned3) %>%
  dplyr::mutate(crisis_sent = paste0(crisno,"_",sentence_number_int_aligned)) %>%
  dplyr::select(crisno, email_crisis_sent_event, do_actor_a, do_actor_b, condition_interact_location, condition_interact_location_other) %>% 
  dplyr::mutate(do_actor_a=strsplit(do_actor_a, ";")) %>% tidyr::unnest(do_actor_a) %>%
  dplyr::mutate(do_actor_b=strsplit(do_actor_b, ";")) %>% tidyr::unnest(do_actor_b) %>%
  dplyr::mutate(condition_interact_location_other=strsplit(condition_interact_location_other, ";")) %>% tidyr::unnest(condition_interact_location_other) %>%
  dplyr::mutate(do_actor_a=tolower(do_actor_a), do_actor_b=tolower(do_actor_b),
                condition_interact_location_other=trimws(tolower(condition_interact_location_other))) %>%
  dplyr::left_join(dictionary_condition_locations %>%
                     dplyr::mutate() %>% 
                     dplyr::mutate(condition_interact_location_other = tolower(condition_interact_location_other)) %>% 
                     dplyr::select(do_actor_a, do_actor_b, condition_interact_location, condition_interact_location_other, norm_name)) %>%
  dplyr::mutate(crisno = as.numeric(crisno)) %>% 
  unique() %>% 
  dplyr::filter(!is.na(condition_interact_location_other))
...
icb_long_condition_location_clean <- icb_long_split %>% 
  dplyr::filter(variable == "condition_interact_location_other") %>% 
  dplyr::full_join(condition_location_merge_directory %>%
                     dplyr::mutate(crisno = as.character(crisno)) %>% 
                     dplyr::rename(value = condition_interact_location_other)) %>% 
  dplyr::rename(value_normalized = norm_name) %>%
  dplyr::mutate(variable_normalized = "condition_interact_location_other_clean") %>%   
  dplyr::select(-do_actor_a, -do_actor_b, -condition_interact_location) %>% 
  dplyr::mutate(value_normalized = ifelse(is.na(value_normalized), "no_cleaning", value_normalized))
...
icb_long_clean <- icb_long_split %>% 
  ...
  dplyr::left_join(icb_long_condition_location_clean, by=names(icb_long_split), suffix = c("", ".condition_location")) %>% 
      dplyr::mutate(variable_normalized = (ifelse(is.na(variable_normalized), variable_normalized.condition_location, variable_normalized))) %>%
      dplyr::mutate(value_normalized = (ifelse(is.na(value_normalized), value_normalized.condition_location, value_normalized))) %>%
      dplyr::select(-variable_normalized.condition_location, -value_normalized.condition_location) %>% 
