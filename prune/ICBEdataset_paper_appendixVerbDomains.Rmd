...
```{r, eval=T, echo=F, results='markup', include=T, message=F, cache=F}

domains <- verbs_sentence_wordnet_top %>% ungroup() %>%  
            dplyr::select(wordnetid) %>%
            mutate(wordnetid=wordnetid %>% as.character() ) %>% 
            left_join(subject_headings_verbs %>% dplyr::select(wordnetid=a,domain=b)) %>%
            count(domain) %>% 
            arrange(desc(n)) %>% 
            na.omit() %>%
  ...
  left_join(
    tree_hypernyms_verbs_used_rooted_table_ready %>%
  dplyr::select(level1,level2) %>% distinct() %>% group_by(level1) %>% summarise(level2=level2 %>% paste0(collapse=" | ") ) %>% dplyr::select(domain=level1,Sysnets=level2)
  )
...
domains_ft <- domains %>%
        ungroup() %>% 
        mutate(Sysnets=Sysnets %>% str_replace_all("[^A-Za-z0-9 ;\\|]","")) %>%
        ...
        set_header_labels( values = list(crisno = "#", input_crisis = "Name", sentence_count="Sentences", word_count="Words", event_count_mean_sum="Events")) %>%
...
domains_ft  %>% saveRDS(paste0(here::here(), '/replication_paper/tables/domains_ft.Rds'))
domains_ft
